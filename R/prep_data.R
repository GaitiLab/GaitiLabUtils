...
#' @export
elbow_method_wrapper <- function(seurat_object, ndims = 50, reduction = "pca") {
    message("Determine number of optimal PCs...")
    npcs <- min(get_pcs(seurat_object))
    p <- Seurat::ElbowPlot(seurat_object, ndims = ndims, reduction = reduction) +
        ggplot2::geom_vline(xintercept = npcs, lty = "dashed", color = "red") +
        ggplot2::annotate("text", x = npcs + 3.5, y = 6, label = glue::glue("Optimal PCs={npcs}")) +
        GBMutils::GBM_theme()
