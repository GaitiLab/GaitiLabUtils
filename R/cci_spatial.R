...
#' @export
inspect_neighbors <- function(cell_id, nn_id_list, group_varname, df_with_labels) {
    if (length(nn_id_list[[cell_id]]) > 0) {
        labels_of_neighbors <- df_with_labels[nn_id_list[[cell_id]], group_varname]
        ...
        labels_of_neighbors <- stringr::str_split(labels_of_neighbors, ",") %>% unlist()
        ...
        return(cbind(
            data.frame(table(labels_of_neighbors)), data.frame(
                cell_id = cell_id, n_total = n_total,
...
#' @return return dataframe with the columns: labels_of_neighbors, cell_id, n_total, and n_cells
#' @export
inspect_k_nearest_neighbors <- function(cell_id, nn_id_mat, group_varname, df_with_labels) {
    ...
    ), data.frame(cell_id = cell_id, total_neighbor_counts = length(labels_of_neighbors))) %>%
        dplyr::rename(
            n_cells = Freq,
        ))
}
...
#' @return return dataframe with the columns: labels_of_neighbors, cell_id, n_total, and n_cells
#' @export
compute_neighbor_metrics <- function(approach, dbscan_res, group_varname, df_with_labels, n_cores = 1, k_neighbors = NA) {
    ...
    if (approach == "knn") {
        ...
        neighbors_df <- do.call(
            rbind,
            parallel::mclapply(names(dbscan_res$id),
                inspect_neighbors,
    ...
    message("Assign cell type to each cell for which the neighbors were inspected...")
    ...
    cell_oi_with_neighbors_df <- cell_oi_with_neighbors_df %>%
        ...
        tidyr::unite(source_target, c(source, target), sep = "__", remove = FALSE) %>%
...
compute_perm_pval <- function(celltype_pair_oi, shuffled_df, obs_df, metric) {
    ...
    means_shuffled <- shuffled_df %>%
        filter(source_target == celltype_pair_oi) %>%
